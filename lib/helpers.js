...
/**
 * Converts Vue single-file-components with
 * <style lang="scss"> / <style lang="sass">
 * blocks into pure SASS blocks with the
 * same line offsets.
 *
 * @param {string} str - The string we wish to parse
 * @returns {array} vueBlocks - Array of Objects with { format: 'sass', text: Buffer... }
 */
helpers.extractVueBlocks = function (str) {
  // Multiple blocks may exist
  var vueBlocks = [];
  var startTag = /\n[ \t]*<style[^>]*lang="(s[ac]ss)"/g;
  var endTag = (/\n[ \t]*<\/style>/);
  var match;
  while ((match = startTag.exec(str)) !== null) {
    var format = match[1];
    var index = match.index;
    var linesBeforeMatch = str.substr(0, index).split(/\r?\n/);
    var linesAfterMatch = str.substr(index).replace(/^\s*/, '');
    // Create a fragment file with blank lines
    // before the matched SASS/SCSS code.
    var fragment = '';
    for (var i = 0; i < linesBeforeMatch.length; i++) {
      fragment += '\n';
    }
    // Comment out the <style> tag
    fragment += '//';
    // Seek the </style> tag
    var match2 = endTag.exec(linesAfterMatch);
    if (match2) {
      fragment += linesAfterMatch.substring(0, match2.index);
      vueBlocks.push({
        'format': format,
        'text': Buffer.from(fragment + '\n')
      });
    }
    else {
      fragment += linesAfterMatch;
      vueBlocks.push({
        'format': format,
        'text': Buffer.from(fragment + '\n')
      });
      // Do not continue processing the file
      break;
    }
  }
  return vueBlocks;
